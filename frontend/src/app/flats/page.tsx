...
function FlatsPageContent() {
  ...
  useEffect(() => {
    document.title = "Flats for Rent | RoomsWallah";
  }, []);
  ...
  useEffect(() => {
    ...
    const paramType = searchParams.get("type") || "flat";
    ...
    if (paramPreferred) {
      ...
      } else if (paramPreferred.toLowerCase() === "family") {
        setSelectedPreferred(["Family / Couple"]);
      } else {
  ...
  // Filter and Sort properties (specific to type flat)
  const filteredProperties = useMemo(() => {
    return properties.filter((item) => {
      if (item.type !== "flat") return false;
      ...
      // Filter by Room Type / BHK Layout
            ...
            <div className={`flex-1 max-w-3xl lg:ml-8 ${showMobileSearch ? "block animate-fadeIn" : "hidden lg:block"}`}>
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-0 border-2 border-black md:border md:border-[#ECECEC] rounded-[4px] md:rounded-[20px] overflow-hidden bg-white shadow-sm">
                ...
                <button 
                  ...
                  className="bg-[#6C4CF1] hover:bg-[#5B3FE6] text-white flex items-center justify-center space-x-1.5 h-11 text-xs font-bold uppercase tracking-wider col-span-1 cursor-pointer"
                >
            ...
            <aside className={`w-full lg:w-[270px] bg-white rounded-[28px] border border-[#ECECEC] p-6 shadow-[0px_4px_16px_rgba(0,0,0,0.02)] space-y-5 shrink-0 text-left ${showMobileFilters ? "block" : "hidden lg:block"}`}>
              ...
              <div className="space-y-3.5 pb-4 border-b border-[#F0F2F5]">
                ...
                {!collapses.preferredFor && (
                  <div className="space-y-2.5 pt-1">
                    {["Boys Only", "Girls Only", "Family / Couple"].map((pref) => {
