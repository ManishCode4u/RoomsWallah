...
async function getProperty(id: string) {
  ...
  console.log(`[hostels-[id]] Fetching URL: "${url}"`);
  try {
    const res = await fetch(url, { cache: "no-store" });
    console.log(`[hostels-[id]] Response status: ${res.status}, ok: ${res.ok}`);
    if (res.ok) {
      ...
      try {
        ...
        console.log(`[hostels-[id]] Error response body: ${text.substring(0, 200)}`);
      } catch (e) {}
    }
  } catch (err) {
