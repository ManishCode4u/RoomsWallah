...
function HostelsPageContent() {
  ...
  useEffect(() => {
    ...
    const paramType = searchParams.get("type") || "hostel";
  ...
  // Filter and Sort properties (specific to type hostel)
  const filteredProperties = useMemo(() => {
    return properties.filter((item) => {
      if (item.type !== "hostel") return false;
                  ...
                  <div className="space-y-3 pt-1">
                    ...
                    <div className="flex justify-between items-center text-[10.5px] text-[#94A3B8] font-bold">
                      ...
                      <span className="text-[#6C4CF1] text-xs">
                        {maxPrice === 200000 ? "Any Price" : maxPrice >= 100000 ? `₹${(maxPrice / 100000).toFixed(1)} Lakh/yr` : `₹${maxPrice.toLocaleString("en-IN")}/yr`}
                      </span>
                    </div>
                  </div>
                )}
