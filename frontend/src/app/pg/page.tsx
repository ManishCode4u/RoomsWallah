...
function PGPageContent() {
  ...
  useEffect(() => {
    document.title = "PGs for Rent | CheckRooms";
  }, []);
  ...
  useEffect(() => {
    ...
    const paramType = typeVal === null ? "pg" : (typeVal === "" ? "all" : typeVal);
          ...
          <div className="flex flex-col lg:flex-row gap-8 items-start">
            ...
            <aside className={`w-full lg:w-[290px] bg-white rounded-[32px] border border-[#ECECEC] p-7 shadow-[0px_4px_24px_rgba(0,0,0,0.02)] space-y-6 shrink-0 text-left lg:sticky lg:top-36 ${showMobileFilters ? "block" : "hidden lg:block"}`}>
              ...
              <div className="space-y-4">
                ...
                {!collapses.amenities && (
                  <div className="space-y-3.5 pt-1">
                    {["Food Included", "Wi-Fi", "AC", "Laundry", "Power Backup"].map((amenity) => (
              ...
              {filteredProperties.length > 0 ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-6">
                  {filteredProperties.map((prop, idx) => {
                    ...
                    const typePath = (prop.type || "pg").toLowerCase();
