...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!property) {
    return {
      title: "Property Not Found | CheckRooms",
      robots: { index: false, follow: false },
    };
  }

  const title = `${property.title} in ${property.area}, ${property.city} - Rent ₹${property.rent}/mo`;
  const description = property.description || `Rent verified ${property.sharing || ""} ${property.type} in ${property.area}, ${property.city} at ₹${property.rent}/month. Zero brokerage, verified listings on CheckRooms.`;
  const canonicalUrl = `${siteUrl}/rooms/${property.id}`;
  ...
  return {
    ...
    keywords: [
      ...
      `rooms in ${property.area}`,
      `rooms near ${property.area}`,
...
export default async function Page({ params }: PageProps) {
  ...
  const accommodationJsonLd = {
    ...
    "@type": "Accommodation",
    "name": property.title,
    "description": property.description || `Rent verified ${property.sharing || ""} ${property.type} at ₹${property.rent}/month in ${property.area}, ${property.city} on CheckRooms.`,
    "url": `${siteUrl}/rooms/${property.id}`,
    "image": imageUrl,
    "address": {
      "@type": "PostalAddress",
  ...
  const breadcrumbJsonLd = {
    ...
    "itemListElement": [
      ...
      },
      {
        ...
        "position": 3,
        "name": property.title,
        "item": `${siteUrl}/rooms/${property.id}`
      }
    ]
  };
