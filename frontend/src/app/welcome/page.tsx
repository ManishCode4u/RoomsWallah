...
export default function WelcomePage() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      if (urlToken) {
        ...
        localStorage.setItem("owner_login_method", "google");
    ...
    try {
      if (activeTab === "login") {
        ...
        if (res.ok) {
          if (typeof window !== "undefined") {
            localStorage.setItem("owner_logged_in", "true");
            localStorage.setItem("owner_login_method", "phone");
            if (data.token) {
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex items-center justify-center p-4 font-manrope text-slate-800">
      
      {/* OLX Styled Login Modal Container Card */}
      <div className="max-w-[430px] w-full bg-white rounded-[24px] border border-slate-200/80 shadow-xl p-6 md:p-8 relative flex flex-col justify-between min-h-[580px] transition-all">
        ...
        <div className={`absolute top-4 flex items-center space-x-1 bg-[#F8FAFC] border border-[#ECECEC] rounded-full p-0.5 z-20 transition-all duration-300 ${
          ...
          <button
            type="button"
            onClick={() => setLang("en")}
            className={`px-2.5 py-0.5 text-[10px] font-semibold uppercase rounded-full transition-all cursor-pointer select-none ${
        ...
        <div>
          {/* Logo container (Brand Home icon & Bold Text Logo) */}
          <div className="w-full flex flex-col items-center justify-center py-4 select-none">
            <div className="w-14 h-14 rounded-2xl bg-gradient-to-tr from-[#6C4CF1] to-[#8E75FF] flex items-center justify-center text-white shadow-md shadow-[#6C4CF1]/25 mb-2.5">
              <Home className="w-7 h-7 stroke-[2.4]" />
            </div>
            <span className="inline-flex items-center font-poppins font-black text-2xl tracking-tight select-none transform scale-y-[1.18] origin-center">
          ...
          {userRole === null ? (
            /* Role Selection Mode */
            <div className="space-y-6 mt-4 text-center animate-fade-in">
              <div>
                <h2 className="font-bold text-xl text-[#1E2235] tracking-tight">
                  {t.selectRole}
                </h2>
              ...
              <div className="space-y-4">
                ...
                >
                  ...
                  <div className="min-w-0 flex-grow">
                    <h4 className="text-base font-semibold text-[#1E2235] tracking-tight">
                      {t.ownerTitle}
                    </h4>
            ...
            <div className="mt-5 transition-all duration-200">
              ...
              <div className="flex border-b border-[#F0F2F5] pb-0.5">
                <button
                  ...
                  className={`flex-1 text-center font-semibold text-sm pb-2 transition-colors cursor-pointer ${
                    activeTab === "login" 
              ...
              <form onSubmit={handleFormSubmit} className="space-y-3.5 text-left mt-4">
                  ...
                  <>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full bg-[#F8FAFC] border border-[#ECECEC] text-[#1E2235] pl-10 pr-4 py-3 rounded-xl text-xs sm:text-sm font-medium focus:outline-none focus:border-[#6C4CF1] focus:bg-white transition-colors"
                      />
                    </div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-[#F8FAFC] border border-[#ECECEC] text-[#1E2235] pl-10 pr-10 py-3 rounded-xl text-xs sm:text-sm font-medium focus:outline-none focus:border-[#6C4CF1] focus:bg-white transition-colors"
                  />
                  <button
                    type="button"
                ...
                {activeTab === "login" && (
                  <div className="flex items-center justify-between py-1">
                    <label className="flex items-center space-x-2 text-xs font-semibold text-slate-500 hover:text-slate-700 cursor-pointer select-none">
                      <input 
                        type="checkbox"
                        checked={rememberMe}
                    ...
                    <Link href="#" className="text-xs font-semibold text-[#6C4CF1] hover:underline">
                      {t.forgotPass}
                    </Link>
                  </div>
                )}
                ...
                <button
                  type="submit"
                  className="w-full bg-[#6C4CF1] hover:bg-[#5B3FE6] text-white font-semibold py-3.5 rounded-xl text-xs sm:text-sm uppercase tracking-wider transition-all duration-200 active:scale-95 shadow-md shadow-[#6C4CF1]/10 cursor-pointer text-center mt-3"
                >
            ...
            <div className="mt-8 transition-all duration-200">
              ...
              <button 
                ...
                className="w-full bg-white hover:bg-slate-50 border-2 border-[#002f34] text-[#002f34] font-semibold h-12 rounded-xl flex items-center justify-start px-4.5 transition-all duration-200 cursor-pointer active:scale-[0.99]"
              >
                <Phone className="w-5 h-5 mr-3 shrink-0 text-[#002f34] stroke-[2]" />
                <span className="flex-grow text-center pr-8 text-sm font-semibold">{t.continuePhone}</span>
              </button>
              ...
              <button 
                ...
                className="w-full bg-white border border-[#ECECEC] hover:bg-slate-50/50 text-slate-700 font-semibold h-12 rounded-xl flex items-center justify-start px-4 transition-all duration-200 cursor-pointer active:scale-[0.99] mt-3"
              >
                ...
                <span className="flex-grow text-center pr-8 text-sm font-semibold">{t.continueGoogle}</span>
              </button>

              {/* OR separator */}
              <div className="text-center font-semibold text-slate-400 text-xs uppercase mt-5.5 tracking-wider select-none">
                {t.or}
              </div>
              ...
              <button 
                onClick={() => setShowPhoneForm(true)}
                className="w-full text-center font-semibold text-[#002f34] text-sm hover:underline underline-offset-4 mt-5.5 cursor-pointer"
              >
                {t.loginPhonePass}
              </button>
            </div>
              ...
              <form onSubmit={handleFormSubmit} className="space-y-3.5 text-left mt-4">
                {activeTab === "signup" && (
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      value={name}
                      onChange={(e) => setName(e.target.value)}
                      className="w-full bg-[#F8FAFC] border border-[#ECECEC] text-[#1E2235] pl-10 pr-4 py-3 rounded-xl text-xs sm:text-sm font-medium focus:outline-none focus:border-[#002f34] focus:bg-white transition-colors"
                    />
                  </div>
                )}
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    value={password}
                    onChange={(e) => setPassword(e.target.value)}
                    className="w-full bg-[#F8FAFC] border border-[#ECECEC] text-[#1E2235] pl-10 pr-10 py-3 rounded-xl text-xs sm:text-sm font-medium focus:outline-none focus:border-[#002f34] focus:bg-white transition-colors"
                  />
                  <button
                    type="button"
                    onClick={() => setShowPassword(!showPassword)}
                ...
                {activeTab === "login" && (
                  <div className="flex items-center justify-between py-1">
                    ...
                    <Link href="#" className="text-xs font-semibold text-[#002f34] hover:underline">
                      {t.forgotPass}
                    </Link>
                  </div>
                )}
                ...
                <button
                  type="submit"
                  className="w-full bg-[#002f34] hover:bg-[#001D20] text-white font-semibold py-3.5 rounded-xl text-xs sm:text-sm uppercase tracking-wider transition-all duration-200 active:scale-95 shadow-md cursor-pointer text-center"
                >
                ...
                <button 
                  ...
                  className="w-full text-center font-semibold text-slate-500 text-xs hover:underline mt-4.5 cursor-pointer block"
                >
                  {t.backOptions}
                </button>
              </form>
        ...
        <div className="mt-8 text-center space-y-4 select-none">
          ...
          <span className="text-xs text-slate-400 font-normal block leading-normal px-4">
            {lang === "en" ? (
              <>
                If you continue, you are accepting CheckRooms{" "}
                <Link href="/terms" className="text-blue-500 hover:underline font-semibold">Terms and Conditions</Link>{" "}
                and{" "}
                <Link href="/privacy-policy" className="text-blue-500 hover:underline font-semibold">Privacy Policy</Link>
              </>
            ) : (
              <>
                यदि आप जारी रखते हैं, तो आप CheckRooms के{" "}
                <Link href="/terms" className="text-blue-500 hover:underline font-semibold">नियम और शर्तें</Link>{" "}
                तथा{" "}
                <Link href="/privacy-policy" className="text-blue-500 hover:underline font-semibold">गोपनीयता नीति</Link>{" "}
                स्वीकार कर रहे हैं
              </>
            )}
          </span>
